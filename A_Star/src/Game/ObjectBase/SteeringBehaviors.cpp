...
Vector2D SteeringBehavior::Calculate()
{
	...
	if (!isSpacePartitioningOn())
	{
		...
		{
			//m_pEntity->World()->TagVehiclesWithinViewRange(m_pEntity, m_dViewDistance);
		}
	}
	else
	{
...
Vector2D SteeringBehavior::CalculateWeightedSum()
{
	...
	if (On(arrive))
	{
		m_vSteeringForce += Arrive(m_vTarget, m_Deceleration) * m_dWeightArrive;
	}
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Evade(const MovingEntity* pursuer)
{
	...
	const double ThreatRange = 400.0;
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Separation(const vector<MovingEntity*> &neighbors)
{
	Vector2D SteeringForce;
...
//  of its neighbors
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Alignment(const vector<MovingEntity*>& neighbors)
{
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Cohesion(const vector<MovingEntity*> &neighbors)
{
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::SeparationPlus(const vector<MovingEntity*> &neighbors)
{
	//Vector2D SteeringForce;
...
//  USES SPACIAL PARTITIONING
//------------------------------------------------------------------------
Vector2D SteeringBehavior::AlignmentPlus(const vector<MovingEntity*> &neighbors)
{
...
//  USES SPACIAL PARTITIONING
//------------------------------------------------------------------------
Vector2D SteeringBehavior::CohesionPlus(const vector<MovingEntity*> &neighbors)
{
	////first find the center of mass of all the agents
	//Vector2D CenterOfMass, SteeringForce;
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Interpose(const MovingEntity* AgentA,
	const MovingEntity* AgentB)
{
...
//
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Hide(const MovingEntity*           hunter,
...
void  SteeringBehavior::NonPenetrationContraint()
{
	for (auto&ent : *MovingEntity::Get_Entities())
	{
		if (ent == m_pEntity)continue;
		{
			Vector2D ToEntity;

			ToEntity = m_pEntity->Pos() - ent->Pos();

			double DistFromEachOther = ToEntity.Length();

			double AmountOfOverLap = ent->BRadius() - DistFromEachOther;
			...
			if (AmountOfOverLap >= 0.0)
			{
				...
				if (DistFromEachOther == 0)
					DistFromEachOther = 1.0;

				newPos = m_pEntity->Pos() + (ToEntity / DistFromEachOther)*AmountOfOverLap;
