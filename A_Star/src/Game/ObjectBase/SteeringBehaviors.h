...
private:
	...
	Vector2D Pursuit(const MovingEntity* agent);

	//this behavior maintains a position, in the direction of offset
	//from the target vehicle
	Vector2D OffsetPursuit(const MovingEntity* agent, const Vector2D offset);
	...
	Vector2D Interpose(const MovingEntity* VehicleA, const MovingEntity* VehicleB);

	//given another agent position to hide from and a list of BaseGameEntitys this
	//method attempts to put an obstacle between itself and its opponent
	Vector2D Hide(const MovingEntity* hunter, const std::vector<BaseGameEntity*>& obstacles);


	// -- Group Behaviors -- //

	Vector2D Cohesion(const std::vector<MovingEntity*> &agents);

	Vector2D Separation(const std::vector<MovingEntity*> &agents);

	Vector2D Alignment(const std::vector<MovingEntity*> &agents);

	//the following three are the same as above but they use cell-space
	//partitioning to find the neighbors
	Vector2D CohesionPlus(const std::vector<MovingEntity*> &agents);
	Vector2D SeparationPlus(const std::vector<MovingEntity*> &agents);
	Vector2D AlignmentPlus(const std::vector<MovingEntity*> &agents);
