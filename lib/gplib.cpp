...
void debug::Draw_ShowFps()
{
	...
	font::Draw_FontTextNC(system::WINW-125,5,0.0f,Buffer,ARGB(255,255,255,255),0);
}
...
static KEYINFO padbase[input::KEY_MAX] =
{
	...
	{ VK_RETURN, input::FREE_KEY, 0 },
	{ JOY_BUTTON5, input::FREE_KEY, 0 }, //BTN_L1   ////////////////////////
	{ JOY_BUTTON6, input::FREE_KEY, 0 }, //BTN_R1  /////////////////////////
	{ JOY_BUTTON7, input::FREE_KEY, 0 }, //BTN_L2   ////////////////////////
	{ JOY_BUTTON8, input::FREE_KEY, 0 }, //BTN_R2  /////////////////////////
...
*/
	...
	v.p = D3DXVECTOR3( posx, posy, z ); // 初期位置
	v.p_size = width; // サイズ
	v.color = color;
	...
	float z, D3DCOLOR color)
{
	...
	CUSTOMVERTEX v[4] = {
		{endX,   startY, z, 1.0f, color}, 
		{endX,   endY, z, 1.0f, color}, 
		{startX, startY, z, 1.0f, color}, 
		{startX, endY, z, 1.0f, color}
	}; 
...
void Draw_HexagonFill(float CenterX, float CenterY, float z, float angle, float HalfSize, D3DCOLOR color)
{
	struct Vertex
	{
		FLOAT x, y, z;
	...
	Vertex v[] =
	{
		{ CenterX,CenterY ,z,1.0f,color },
		{ CenterX + std::cos(D3DXToRadian(90.f + angle))*HalfSize,CenterY - std::sin(D3DXToRadian(90.f + angle))*HalfSize,z,1.0f,color },
		{ CenterX + std::cos(D3DXToRadian(30.f + angle))*HalfSize,CenterY - std::sin(D3DXToRadian(30.f + angle))*HalfSize,z,1.0f,color },
		{ CenterX + std::cos(D3DXToRadian(-30.f + angle))*HalfSize,CenterY - std::sin(D3DXToRadian(-30.f + angle))*HalfSize,z,1.0f,color },
		{ CenterX + std::cos(D3DXToRadian(-90.f + angle))*HalfSize,CenterY - std::sin(D3DXToRadian(-90.f + angle))*HalfSize,z,1.0f,color },
		{ CenterX + std::cos(D3DXToRadian(-150.f + angle))*HalfSize,CenterY - std::sin(D3DXToRadian(-150.f + angle))*HalfSize,z,1.0f,color },
		{ CenterX + std::cos(D3DXToRadian(-210.f + angle))*HalfSize,CenterY - std::sin(D3DXToRadian(-210.f + angle))*HalfSize,z,1.0f,color },
		{ CenterX + std::cos(D3DXToRadian(90.f + angle))*HalfSize,CenterY - std::sin(D3DXToRadian(90.f + angle))*HalfSize,z,1.0f,color }
	};
	pD3DDevice->SetTexture(0, NULL);
	DWORD fvf = 0;
	pD3DDevice->GetFVF(&fvf);
	pD3DDevice->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
	pD3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 6, v, sizeof(Vertex));
	pD3DDevice->SetFVF(fvf);
}
void graph::Draw_Hexagon(float CenterX, float CenterY, float z, float angle, int LineSize, float HalfSize, D3DCOLOR FrameColor, D3DCOLOR FillColor, bool DoFill)
{
	HalfSize *= camera::camera->changeScale(1.f);
	CenterX = camera::camera->xchange(CenterX);
	CenterY = camera::camera->ychange(CenterY);
	if (DoFill)
	{
		DD_ReDrawCheck(FillColor);
		Draw_HexagonFill(CenterX, CenterY, z, angle , HalfSize, FillColor);
	}
	...
	for (int i = 0; i < LineSize; i++)
	{
		...
		Vertex v[] =
		{
			{ CenterX + std::cos(D3DXToRadian(90.f + angle))*(HalfSize - i / 2),CenterY - std::sin(D3DXToRadian(90.f + angle))*(HalfSize - i / 2),z,1.0f,FrameColor },
			{ CenterX + std::cos(D3DXToRadian(30.f + angle))*(HalfSize - i / 2),CenterY - std::sin(D3DXToRadian(30.f + angle))*(HalfSize - i / 2),z,1.0f,FrameColor },
			{ CenterX + std::cos(D3DXToRadian(-30.f + angle))*(HalfSize - i / 2),CenterY - std::sin(D3DXToRadian(-30.f + angle))*(HalfSize - i / 2),z,1.0f,FrameColor },
			{ CenterX + std::cos(D3DXToRadian(-90.f + angle))*(HalfSize - i / 2),CenterY - std::sin(D3DXToRadian(-90.f + angle))*(HalfSize - i / 2),z,1.0f,FrameColor },
			{ CenterX + std::cos(D3DXToRadian(-150.f + angle))*(HalfSize - i / 2),CenterY - std::sin(D3DXToRadian(-150.f + angle))*(HalfSize - i / 2),z,1.0f,FrameColor },
			{ CenterX + std::cos(D3DXToRadian(-210.f + angle))*(HalfSize - i / 2),CenterY - std::sin(D3DXToRadian(-210.f + angle))*(HalfSize - i / 2),z,1.0f,FrameColor },
			{ CenterX + std::cos(D3DXToRadian(90.f + angle))*(HalfSize - i / 2),CenterY - std::sin(D3DXToRadian(90.f + angle))*(HalfSize - i / 2),z,1.0f,FrameColor }
		};
		DD_ReDrawCheck(FrameColor);
		pD3DDevice->SetTexture(0, NULL);
		...
		pD3DDevice->GetFVF(&fvf);
		pD3DDevice->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
		pD3DDevice->DrawPrimitiveUP(D3DPT_LINESTRIP, 6, v, sizeof(Vertex));
		pD3DDevice->SetFVF(fvf);
	}
}
...
void graph::Draw_Triangle(float CenterX, float CenterY, float z, float angle, float Width, float Height, D3DCOLOR Color)
{
	Width *= camera::camera->changeScale(1.f);
	Height *= camera::camera->changeScale(1.f);
	CenterX = camera::camera->xchange(CenterX);
	CenterY = camera::camera->ychange(CenterY);
	...
	Vertex v[]=
	{
		{CenterX + cos(D3DXToRadian(angle))*((Height / 3.f)*2.f),CenterY + sin(D3DXToRadian(angle))*((Height / 3.f)*2.f),z,1.0f,Color },
		{(CenterX + cos(D3DXToRadian(180.f + angle))*(Height / 3.f)) + cos(D3DXToRadian(270.f + angle))*Width / 2.f,(CenterY + sin(D3DXToRadian(180.f + angle))*(Height / 3.f)) + sin(D3DXToRadian(270.f + angle))*Width / 2.f,z,1.0f,Color},
		{ (CenterX + cos(D3DXToRadian(180.f + angle))*(Height / 3.f)) - cos(D3DXToRadian(270.f + angle))*Width / 2.f,(CenterY + sin(D3DXToRadian(180.f + angle))*(Height / 3.f)) - sin(D3DXToRadian(270.f + angle))*Width / 2.f,z,1.0f,Color }
	};
	...
	pD3DDevice->GetFVF(&fvf);
	pD3DDevice->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
	pD3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLELIST, 1, v, sizeof(Vertex));
	pD3DDevice->SetFVF(fvf);
}
...
void graph::Draw_Crcle(float CenterX, float CenterY, float z, float Radius,double surface, D3DCOLOR FillColor)
{
	...
	v[0] = { CenterX, CenterY, 0.f, 1.0f, FillColor };

	for (int i = 360; i >= 1; --i)
		v[i] = { CenterX + Radius*std::sin(D3DXToRadian(i - 180)), CenterY + Radius*std::cos(D3DXToRadian(i - 180)), 0.f, 1.0f, FillColor };
		...
		pD3DDevice->GetFVF(&fvf);
		pD3DDevice->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
		pD3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, surface, v, sizeof(Vertex));
		pD3DDevice->SetFVF(fvf);
...
void graph::Draw_Circle(float CenterX, float CenterY, float z, float Radius,D3DCOLOR FillColor)
{
	CenterX = camera::camera->xchange(CenterX);
	CenterY = camera::camera->ychange(CenterY);
	...
	pD3DDevice->GetFVF(&fvf);
	pD3DDevice->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
	pD3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 359, v, sizeof(Vertex));
	pD3DDevice->SetFVF(fvf);
...
/////////////////////線だけの円//////////////////////////////
void graph::Draw_CrcleLine(float CenterX, float CenterY,float z, float Radius, D3DCOLOR FrameColor)
{
	...
	for (int i = 0; i < 360; i++)
	{
		v[i] = { CenterX + Radius*std::cos(D3DXToRadian(i - 1)), CenterY + Radius*std::sin(D3DXToRadian(i - 1)), 0.f, 1.0f, FrameColor };
	}
	...
	pD3DDevice->GetFVF(&fvf);
	pD3DDevice->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
	pD3DDevice->DrawPrimitiveUP(D3DPT_LINESTRIP, 359, v, sizeof(Vertex));
	pD3DDevice->SetFVF(fvf);

}
void graph::Draw_Line(float StartX, float StartY, float EndX, float EndY)
{
	StartX = camera::camera->xchange(StartX);
	StartY = camera::camera->ychange(StartY);
	EndX = camera::camera->xchange(EndX);
	EndY = camera::camera->ychange(EndY);
	...
	Vertex v[2] =
	{
		{StartX,StartY,0.f,1.0f,ARGB(255,255,0,255)},
		{EndX,EndY,0.f,1.0f,ARGB(255,255,0,255)}
	};
	pD3DDevice->SetTexture(0, NULL);
	DWORD fvf = 0;
	pD3DDevice->GetFVF(&fvf);
	pD3DDevice->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
	pD3DDevice->DrawPrimitiveUP(D3DPT_LINESTRIP, 1, v, sizeof(Vertex));
	pD3DDevice->SetFVF(fvf);
...
//	sprintf_s(buf, "%s/%04d%02d%02d%02d%02d%02d%lu.png", path.c_str(), year, month, day, hour, minute, second, oTime);
	...
	pD3DDevice->GetFrontBufferData(0, pSurface);   //サーフェイスを作ったらデータ取得
