...
#define ARGB D3DCOLOR_ARGB
		  ...
		  void Draw_SetRenderMode(int Mode, bool d2d = true);
      void Draw_EndRenderMode(bool d2d = true);
        ...
        float	degree = 0, POINT *center = nullptr,
		  ...
		  void Draw_Hexagon(float CenterX, float CenterY, float z, float angle, int LineSize, float HalfSize, D3DCOLOR FrameColor,D3DCOLOR FillColor,bool DoFill);
		  void Draw_Triangle(float CenterX, float CenterY, float z, float angle,float Width,float Height, D3DCOLOR Color);
		  void Draw_Line(float StartX, float StartY, float EndX, float EndY);
		  void Draw_Crcle(float CenterX, float CenterY, float z, float Radius,double surface, D3DCOLOR FillColor);
		  void Draw_Circle(float CenterX, float CenterY, float z, float Radius,D3DCOLOR FillColor);
		  void Draw_CrcleLine(float CenterX, float CenterY, float z, float Radius, D3DCOLOR FrameColor);
		  void Draw_LineNC(
			  int startX, int startY,
			  int endX, int endY,
			  float z, D3DCOLOR color, int size
      ...
      bool Draw_3DUpdate();
		  ...
		  unsigned int Draw_GetImageHeight(const std::string& resname);
			...
			void Draw_BillBoard(float posx, float posy, float width, float height, float z, std::string texname,D3DCOLOR color = ARGB(255,255,255,255));
			void SetMtrl(int a = 255, int r = 255, int g = 255, int b = 255);
			...
			struct Itapoly{
				...
				Itapoly(const D3DXVECTOR3& inpos, DWORD incolor, const D3DXVECTOR2& inuv)
					:
					pos(inpos),
					rhw(1.0f),
					color(incolor),
